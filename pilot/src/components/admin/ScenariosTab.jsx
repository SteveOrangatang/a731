...
export default function ScenariosTab({ scenarios, agents, onUpsert, onDelete }) {
  ...
  const startNew = () => {
    setDraft({ ...EMPTY, id: `lesson-${Date.now()}`, order: keys.length });
    setEditing('new');
  };
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                  ...
                  <button
                    onClick={() => {
                      if (
                        window.confirm(
                          `Delete "${scenario.title}"? This also deletes the ${personasCount} personas attached to it.`,
                        )
                      )
                        onDelete(key);
                    }}
...
function ScenarioEditor({ draft, setDraft, onCancel, onSave }) {
  return (
    <div className="p-5 bg-emerald-50 border-b space-y-3">
      ...
      <div>
        <label className="block text-xs font-semibold text-slate-500 mb-1">
          ID (used by personas as lessonId — change with care)
        </label>
        <input
          value={draft.id}
