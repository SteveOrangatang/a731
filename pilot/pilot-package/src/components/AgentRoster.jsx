...
/**
 * Stage-grouped roster (mirrors the production simulator):
 *   Stage 1 INTAKE        — the leader (initial briefing)
 *   Stage 2 INVESTIGATION — four subordinates
 *   Stage 3 BRIEFBACK     — the leader again, after enough subordinates engaged
 *
 * Archetype labels (Loyalist/Operator/Stickler/Partner) are intentionally
 * hidden from the roster — they live in the lesson handout instead.
 *
 * stageLocks shape:
 *   { stage2Unlocked, stage3Unlocked, requiredEngagement, totalSubordinates }
 */
      ...
      <div className="flex-grow overflow-y-auto p-2 space-y-4">
        {leader && (
          ...
          >
            <AgentRow
              agent={leader}
              selected={selectedAgent?.personaId === leader.personaId}
              onSelect={onSelect}
            />
          </StageGroup>
        )}
        ...
        >
          {subordinates.map((agent) => (
            <AgentRow
              key={agent.personaId}
              agent={agent}
              selected={selectedAgent?.personaId === agent.personaId}
        ...
        {leader && (
          ...
          >
            <AgentRow
              agent={leader}
              selected={selectedAgent?.personaId === leader.personaId && stage3Unlocked}
