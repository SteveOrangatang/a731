...
function scenarioOptions(scenarios) {
  return Object.values(scenarios || {})
    ...
    .map((s) => ({ id: s.scenarioId, label: s.title || s.scenarioId }));
}
...
export default function PersonasTab({
  ...
  const filtered = useMemo(() => {
    const f =
      filter === 'all'
        ? agents
        : agents.filter((a) => a.scenarioId === filter);
    return [...f].sort((a, b) => {
  ...
  const startNew = () => {
    setDraft({
      ...
      scenarioId: lessonOpts[0]?.id || 'lesson1',
    });
    setEditing('new');
  };
  ...
  const save = async () => {
    if (!draft.name.trim() || !draft.personaId.trim()) return;
    try {
  ...
  return (
    <div>
      ...
      <ul className="divide-y">
        {filtered.map((agent) => {
          const lessonLabel =
            scenarios?.[agent.scenarioId]?.title ||
            agent.scenarioId ||
            '—';
          return (
            <li
            ...
            >
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                  <button
                    onClick={() => onToggle(agent.personaId)}
                  ...
                  <button
                    onClick={() => {
                      if (window.confirm('Mark this persona inactive? (v0.0.3 has no hard delete; this toggles active off.)'))
                        onDelete(agent.personaId);
                    }}
                    className="p-2 text-slate-300 hover:text-red-600 hover:bg-red-50 rounded"
                    title="Delete (soft — toggles inactive)"
                  >
    ...
    <div className="p-5 bg-emerald-50 border-b space-y-3">
      <div className="grid grid-cols-3 gap-3">
        ...
        <Input
          ...
          value={draft.personaType}
          onChange={(v) => setDraft({ ...draft, personaType: v })}
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Input
          label="Persona ID (do not change unless you know why)"
          value={draft.personaId}
          onChange={(v) => setDraft({ ...draft, personaId: v })}
          mono
        />
        <div>
          <label className="block text-xs font-semibold text-slate-500 mb-1">
